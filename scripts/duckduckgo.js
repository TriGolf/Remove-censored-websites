...
async function testUrl(result) {
    try {
        console.log(result.querySelector('[data-testid="result-extras-url-link"').getAttribute("href"))
        const response = await fetch(result.querySelector('[data-testid="result-extras-url-link"').getAttribute("href"));
    } catch (e) {
...
function removeBlockedUrls() {
    browser.storage.local.get("extensionEnabled", (data) => {
        if (data.extensionEnabled) {
            ...
            waitForElement('[data-testid="result"]', async (element) => {
